...
import copy
f6 = open('./benign_graph/path_benign.txt', 'w')
...
def graph_add_node_realapt(g:nx.DiGraph, logs, key, md5_to_node:dict):
    ...
    if key == APTLOG_KEY.FILE:
        for index, row in logs.iterrows():
            ...
            node_set.add(s_node)
            node_set.add(t_node)
            edge_set.add((s_node, t_node, e_id, sys_type, proc_name, tech_num, anomaly_socre))
    ...
    for edge in edge_list:
        g.add_edge(edge[0], edge[1], e_id = edge[2], sys_type = edge[3], proc_name = edge[4], tech_num = edge[5], anomaly_socre = edge[6])
...
def backtrack(node, g:nx.DiGraph, store:nx.DiGraph, s1:str):
    if list(g.predecessors(node)):
            for n in g.predecessors(node):
                ...
                store.add_edge(n, node, e_id=g[n][node]['e_id'], sys_type=g[n][node]['sys_type'], proc_name=g[n][node]['proc_name'], tech_num=g[n][node]['tech_num'], anomaly_socre=g[n][node]['anomaly_socre'])
                k = ''
...
def forwardtrack(node, g:nx.DiGraph, store:nx.DiGraph):
    if list(g.successors(node)):
            for n in g.successors(node):
                ...
                store.add_edge(node, n, e_id=g[node][n]['e_id'], sys_type=g[node][n]['sys_type'], proc_name=g[node][n]['proc_name'], tech_num=g[n][node]['tech_num'], anomaly_socre=g[n][node]['anomaly_socre'])
                forwardtrack(n, g, store)


if __name__ == "__main__":
    log = read_org_log_from_json("./benigntag.json")
    g = graph_init()
    node = {}
    print("start")
    ...
    truth = []
    f1 = open("./1.5+cmdline+benign.txt", 'r', encoding='utf-8')
...
##    f3 = open("./road_str2.txt", 'w')
    f4 = open("./benign_graph/not_in_graph_benign.txt", 'w')
    f5 = open("./benign_graph/road_whole_benign.txt", 'w')
    graph_list = []
    s_list = []
    allnum = 0
    continuenum = 0
...
#    print(truth)
    ...
    for n in truth:
        ...
        allnum += 1
        if(n not in g):
            f4.write("line NO." + str(linei) + ":" + str(n) + "\n")
            linei += 1
            continuenum += 1
            continue
#        print(n)
