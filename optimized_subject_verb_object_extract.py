...
def _get_subs_from_conjunctions(subs):
    ...
    for sub in subs:
        ...
        if contains_conj(rightDeps):
            more_subs.extend([tok for tok in rights if tok.dep_ in SUBJECTS or tok.pos_ == "NOUN" or tok.pos_ == "PROPN"])
            if len(more_subs) > 0:
                more_subs.extend(_get_subs_from_conjunctions(more_subs))
    return more_subs


# get verbs joined by conjunctions
def _get_verbs_from_conjunctions(verbs):
    more_verbs = []
    for verb in verbs:
        # rights is a generator
        ...
        rightDeps = {tok.lower_ for tok in rights}
        if contains_conj(rightDeps):
            more_verbs.extend([tok for tok in rights if tok.dep_ in SUBJECTS or tok.pos_ == "VERB"])
            if len(more_verbs) > 0:
                more_verbs.extend(_get_subs_from_conjunctions(more_verbs))
    return more_verbs
...
def _get_objs_from_conjunctions(objs):
    ...
    for obj in objs:
        ...
        rightDeps = {tok.lower_ for tok in rights}
        print('000',rightDeps)
        print([(tok.lower_, tok.dep_, tok.pos_) for tok in rights])
        if contains_conj(rightDeps):
            more_objs.extend([tok for tok in rights if tok.dep_ in OBJECTS or tok.pos_ == "NOUN" or tok.pos_ == "PROPN"])
            if len(more_objs) > 0:
...
def _get_all_objs(v, is_pas):
    ...
    for tok in objs:
        for tmp in tok.rights:
            if tmp.pos_ == "NOUN" or tmp.pos_ == "PROPN":
                objs.append(tmp)
    ...
    print(potential_new_verb, potential_new_objs)
    if potential_new_verb != None:
        vodic[potential_new_verb] = potential_new_objs
    print('111',vodic)
...
def expand(item, tokens, visited):
    ...
    if hasattr(item, 'lefts'):
        for part in item.lefts:
            if part.pos_ in BREAKER_POS or part.pos_ == 'PUNCT':
                break
            if not part.lower_ in NEGATIONS:
                parts.append(part)
                #下两行可要可不要
                for i in part.rights:
                    parts.append(i)
    parts.append(item)
    print(parts)
    ...
    if hasattr(parts[-1], 'rights'):
        ...
        if parts[-1].pos_ == 'ADP':
            ...
            tmp = parts[-1]
            while(hasattr(tmp, 'rights')):
                flag = 0
                for item3 in tmp.rights:
                    print(item3.lower_, item3.pos_)
                    if item3.pos_ == "NOUN" or item3.pos_ == "PROPN":
                        flag = 1
                        parts.append(item3)
                    tmp = item3
                    break
...
def findSVOs(tokens):
    ...
    for tok in tokens:
        print(tok.lower_, tok.dep_, tok.pos_, list(tok.lefts), list(tok.rights), tok.head.text)
    ...
    for v in verbs:
        ...
        print(v, subs, verbNegated)
        isConjVerb, conjV = _right_of_verb_is_conj_verb(v)

        print(v, isConjVerb, conjV, v.dep_, v.pos_, list(v.lefts), list(v.rights))

        # hopefully there are subs, if not, don't examine this verb any longer
        if len(subs) > 0:
            isConjVerb, conjV = _right_of_verb_is_conj_verb(v)

            print(v, isConjVerb, conjV)

            if isConjVerb:
                vodic = _get_all_objs(conjV, is_pas)
                vl = vodic.keys()
                print(vl)
                for v2 in vl:
                    objs = vodic[v2]
                            ...
                            for obj in objs:
                                ...
                                if is_pas:  # reverse object / subject for passive
                                    ...
                                    svos.append((to_str(expand(sub, tokens, visited)),
                                                "!" + v.lemma_ if verbNegated or objNegated else v.lemma_, to_str(expand(obj, tokens, visited))))
                                    svos.append((to_str(expand(sub, tokens, visited)),
                                                "!" + v2.lemma_ if verbNegated or objNegated else v2.lemma_, to_str(expand(obj, tokens, visited))))
                        else:
                            # no obj - just return the SV parts
                            svos.append((to_str(expand(sub, tokens, visited)),
                                         "!" + v.lemma_ if verbNegated else v.lemma_, ''))    
            else:
                vodic = _get_all_objs(conjV, is_pas)
                vl = vodic.keys()
                for v in vl:
            ...
            v_new = v.head
            subs, verbNegated = _get_all_subs(v_new)
            if len(subs) > 0:
                isConjVerb, conjV = _right_of_verb_is_conj_verb(v)
                print(v, isConjVerb, conjV)
                if isConjVerb:
        ...
        '''
            ...
            if isConjVerb:
                ...
                for obj in objs:
                    ...
                    if is_pas:  # reverse object / subject for passive
                        svos.append((to_str(expand(obj, tokens, visited)),
                                     "!" + v.lemma_ if verbNegated or objNegated else v.lemma_, sub))
                        svos.append((to_str(expand(obj, tokens, visited)),
                                     "!" + v2.lemma_ if verbNegated or objNegated else v2.lemma_, sub))
                    else:
                        svos.append((sub,
                                     "!" + v.lemma_ if verbNegated or objNegated else v.lower_, to_str(expand(obj, tokens, visited))))
                        svos.append((sub,
                                     "!" + v2.lemma_ if verbNegated or objNegated else v2.lower_, to_str(expand(obj, tokens, visited))))
            else:
                v, objs = _get_all_objs_t(v, is_pas)
                sub = ''
                if len(objs) > 0:
                    for obj in objs:
                        objNegated = _is_negated(obj)
