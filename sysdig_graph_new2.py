...
def graph_add_node_realapt(g:nx.DiGraph, logs, key, md5_to_node:dict):
    ...
    if key == APTLOG_KEY.FILE:
        ...
        for index, row in logs.iterrows():
            s_node = get_md5(row['proc.ppcmdline'])
            t_node = get_md5(row['proc.cmdline'])
            if s_node not in md5_to_node:
                md5_to_node[s_node] = row['proc.ppcmdline']
            if t_node not in md5_to_node:
...
if __name__ == "__main__":
    ffff1 = sys.argv[1] 
    log = read_org_log_from_json(ffff1)
    g = graph_init()
    node = {}
    ...
    ffff = sys.argv[2]
    thres = sys.argv[3]
    f1 = open(ffff, 'r', encoding='utf-8')
    for line in f1.readlines():
        #line = line.split(',')
        #if(line[1] == '1'):
        #   truth.append(line[0])
        line = json.loads(line)
        if float(line['anomaly_socre']) + 0.000001 > float(thres):
            cmddd = line['proc.cmdline']
        #line[0] = line[0].replace('\n', '')
            #cmddd = cmddd.replace('++', '\\')
            #cmddd = cmddd.replace('++', '\\')
#        line = line.replace('_', '$')
            truth.add(get_md5(cmddd))
    f1.close()
    fbenign = open("./benign_tag/log-20h_tag.json", 'r')
    benignset = set()
    for line in fbenign.readlines():
        line = json.loads(line)
        if float(line['anomaly_socre']) + 0.000001 > float(thres):
            cmddd = line['proc.cmdline']
            benignset.add(get_md5(cmddd))
    fbenign.close()
    with open('./benign_tag/benigncmd_higher_thres.pkl', 'wb') as fbenignbbb:
        pickle.dump(benignset, fbenignbbb)
    truth = truth - benignset
    print(truth)
    for i in truth:
        print(node[i])
##    f2 = open("./road2.txt", 'w')
##    f3 = open("./road_str2.txt", 'w')
    ...
    f5 = open("road_whole_"+ str(ffff1).split('/')[1] +".txt", 'w')
    graph_list = []
    s_list = []
    allnum = 0
    continuenum = 0
#    print(len(truth))
#    print(truth)IyAtKi0gY29kaW5nOiB1dGYtOCAtKi0KaW1wb3J0IHN5cwppbXBvcnQgc29ja2V
    k = 0
    linei = 1
    g1 = nx.DiGraph()
    for n in truth:
...
#        print(s)
    ...
    print(allnum)
    print(continuenum)
    #print(len(nx.weakly_connected_components(g1)))
    print('ttttttttttttt')
    nnnnnum = 0 
